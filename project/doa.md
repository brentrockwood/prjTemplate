...
- If starting a new project
    ...
    - Copy the `scripts/` directory from the DOA repository into `project/scripts/` if it doesn't already exist. These scripts manage `context.md` and must be present before the first entry is added.
    - Add the first context entry: `project/scripts/add-context --agent "<agent>" --model "<model>" "Started <projectname> project."` The body can be brief at this stage. This is our ongoing work log. Entries *must* never be edited after they have been entered. Consider it an auditable artifact.
...
- If resuming an existing project
    ...
    - Read the last entry or two from `project/context.md` using the script: `project/scripts/read-context -n 2`. You do not need to read the whole file — just enough to know where we left off in the previous session. Cap at 12K of text at the most. If you do not have enough context from that, ask for help. If no context exists because we are using these standards for the first time on an existing project, copy the scripts into `project/scripts/`, review the existing codebase, and add an entry that summarizes the current state as well as you can. This is our ongoing work log. Entries *must* never be edited after they have been entered. Consider it an auditable artifact.
...
- **Body input methods (in priority order):**
  1. Direct argument: `add-context --agent "..." --model "..." "Entry text"`
  2. From file: `add-context --agent "..." --model "..." --file body.txt`
  3. From stdin: `echo "Entry" | add-context --agent "..." --model "..."`
...
1. **Add context entry** (ALWAYS)
   ```bash
   project/scripts/add-context --agent "<agent>" --model "<model>" "Summary of what was done, what changed, what's next"
   ```
...
- Create the entry immediately: `project/scripts/add-context --agent "..." --model "..." "Retroactive entry for interaction at [approximate time]: [what was done]"`
...
2. `project/scripts/read-context -n 2` to resume context
3. Create feature branch: `git checkout -b feature-name`
4. Implement following DOA phases
5. Add context entry after each interaction

**Completing an interaction:**
1. `project/scripts/add-context --agent "..." --model "..." "Summary"`
2. Run security scan if files changed
...
If you haven't figured it out yet, these rules are designed to help us work together most efficiently, with the fewest surprises, and the smallest blast radius when things inevitably go wrong. Just remember, if you think you know something I don't, tell me. If you think I know something you don't or you have a question, stop and ask.
